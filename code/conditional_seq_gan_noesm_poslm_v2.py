...
class Generator(nn.Module):
    ...
    def sample_gumbel_soft(self, bg_vec: torch.Tensor, tau: float = 1.0, hard: bool = False) -> torch.Tensor:
        B = bg_vec.size(0)
        ...
        y = nn.functional.gumbel_softmax(logits, tau=tau, hard=hard, dim=-1)
...
class ConditionalSequenceDisc(nn.Module):
    ...
    def forward_features(self, tokens_or_soft: torch.Tensor, bg_vec: torch.Tensor, is_soft: bool = False) -> torch.Tensor:
        """Return pre-logit feature vector for feature matching loss."""
        x = self._embed(tokens_or_soft, is_soft=is_soft)
        x = x.transpose(1, 2)
        feats = []
        for block in self.convs:
            h = block(x)
...
def hinge_g_adv(logit_fake: torch.Tensor) -> torch.Tensor:
    """Standard non-saturating G loss: maximize D(fake) score."""
    return -logit_fake.mean()


def compute_pos_freq(tokens_array: np.ndarray, center_pos: int = CENTER_POS) -> np.ndarray:
    """Compute empirical position-specific AA frequency from positive sequences. Returns (L, V) float32."""
    N, L = tokens_array.shape
    counts = np.zeros((L, VOCAB_SIZE), dtype=np.float64)
    for i in range(N):
        for j in range(L):
            t = int(tokens_array[i, j])
            if t != PAD_IDX and j != center_pos:
                counts[j, t] += 1.0
    denom = counts.sum(axis=1, keepdims=True).clip(min=1.0)
    return (counts / denom).astype(np.float32)


def pos_dist_loss(fake_soft: torch.Tensor, pos_freq: torch.Tensor,
                  center_pos: int = CENTER_POS) -> torch.Tensor:
    """CE between G output distribution and empirical positive AA frequencies.
    Directly pushes G to generate positive-like amino acids at each position.
    fake_soft: (B, L, V), pos_freq: (L, V) on same device."""
    mask = torch.ones(fake_soft.size(1), device=fake_soft.device)
    mask[center_pos] = 0.0
    log_prob = torch.log(fake_soft + 1e-8)  # (B, L, V)
    target = pos_freq.unsqueeze(0)           # (1, L, V)
    ce = -(target * log_prob).sum(-1)        # (B, L)
    return (ce * mask.unsqueeze(0)).mean()
...
def main():
    ...
    ap.add_argument("--lambda-feat-match", type=float, default=0.5,
                    help="Feature matching loss weight (0=disable).")
    ap.add_argument("--tau-min", type=float, default=0.3,
                    help="Min Gumbel-softmax temperature; anneals from --tau to this over adv epochs.")
    ap.add_argument("--n-d-steps", type=int, default=2,
                    help="Number of D update steps per G update step.")
    ap.add_argument("--lambda-pos-dist", type=float, default=0.5,
                    help="Positive distribution CE loss weight. Pushes G to match empirical positive AA frequencies.")
    ap.add_argument("--mask-ratio", type=float, default=0.30,
                    help="MLM mask ratio for G warmup and LM loss during GAN training.")
    ...
    ap.add_argument("--lambda-contrast", type=float, default=0.0,
                    help="Negative contrast loss weight: penalizes G when D(fake) < D(neg).")
    ap.add_argument("--contrast-margin", type=float, default=0.1,
                    help="Margin for negative contrast loss: relu(D(neg) - D(fake) + margin).")
    ap.add_argument("--no-d-neg", action="store_true",
                    help="D trains only on pos vs fake; real negatives not used in D loss.")
    ...
    # Precompute positive sequence position-specific AA frequency for PSD loss
    if args.lambda_pos_dist > 0.0:
        print(f'[INFO] computing pos_freq from {pos_idx.size} positive sequences...')
        pos_freq_np = compute_pos_freq(ds.tokens[pos_idx])
        pos_freq_tensor = torch.from_numpy(pos_freq_np).to(device)
        print(f'[INFO] pos_freq computed, shape={pos_freq_np.shape}')
    else:
        pos_freq_tensor = None
    ...
    step = 0
    bg_miss_total = 0
    # tau annealing over adversarial training epochs
    _adv_start_epoch = args.warmup_epochs + args.d_warmup_epochs + 1
    _total_adv_epochs = max(1, args.epochs - _adv_start_epoch + 1)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            for _ in range(steps_per_epoch):
                ...
                if epoch <= args.warmup_epochs:
                    ...
                    lm, logits, gamma, beta = seq_mlm_loss(G, pos_tok, pos_bg, mask_ratio=args.mask_ratio, debug_return_logits=True)
                ...
                # generate fake sequences for D training using hard (discrete) tokens — matches eval
                G.eval()
                with torch.no_grad():
                    fake_st_d = G.sample_gumbel_soft(fake_bg, tau=args.tau, hard=True)
                    fake_hard_idx = fake_st_d.argmax(dim=-1)  # (B, L) integer indices

                # ---- D step ----
                D.train()
                optD.zero_grad(set_to_none=True)

                logit_fake = D(fake_hard_idx, fake_bg, is_soft=False)
                ...
                _logit_neg_for_d = None if args.no_d_neg else logit_neg
                loss_D = hinge_d_loss(logit_pos, _logit_neg_for_d, logit_fake)
                ...
                # tau annealing: decay linearly over adversarial epochs
                _adv_prog = max(0.0, epoch - _adv_start_epoch) / _total_adv_epochs
                current_tau = max(args.tau_min, args.tau - (args.tau - args.tau_min) * _adv_prog)

                # G always conditions on positive-sample backgrounds for adversarial step
                pb_g = next(pos_iter)
                pos_tok_g = pb_g["tokens"].to(device)
                pos_bg_g = pb_g["bg"].to(device)
                bg_miss_total += int(pb_g.get("_bg_miss", 0))

                # STE: one-hot forward (consistent with eval) + soft gradients
                fake_st = G.sample_gumbel_soft(pos_bg_g, tau=current_tau, hard=True)
                logit_fake_for_g = D(fake_st, pos_bg_g, is_soft=True)
                # Separate soft sample for PSD (log(one-hot) is unstable)
                fake_soft_psd = G.sample_gumbel_soft(pos_bg_g, tau=current_tau, hard=False)

                adv = hinge_g_adv(logit_fake_for_g)
                center = center_penalty(fake_st, weight=1.0)
                lm = seq_mlm_loss(G, pos_tok_g, pos_bg_g, mask_ratio=args.mask_ratio)

                # Feature matching: align fake intermediate features to real positive features
                if args.lambda_feat_match > 0.0:
                    D.eval()
                    with torch.no_grad():
                        feat_pos = D.forward_features(pos_tok_g, pos_bg_g, is_soft=False).detach()
                    D.train()
                    feat_fake = D.forward_features(fake_st, pos_bg_g, is_soft=True)
                    feat_match = F.mse_loss(feat_fake.mean(0), feat_pos.mean(0))
                else:
                    feat_match = torch.zeros(1, device=device)

                # Positive distribution loss: push G output to match empirical positive AA frequencies
                if args.lambda_pos_dist > 0.0 and pos_freq_tensor is not None:
                    psd = pos_dist_loss(fake_soft_psd, pos_freq_tensor)
                else:
                    psd = torch.zeros(1, device=device)

                # Negative contrast loss: push D(fake) above D(neg) + margin
                if args.lambda_contrast > 0.0:
                    nb_g = next(neg_iter)
                    neg_tok_g = nb_g["tokens"].to(device)
                    neg_bg_g  = nb_g["bg"].to(device)
                    with torch.no_grad():
                        logit_neg_g = D(neg_tok_g, neg_bg_g, is_soft=False)
                    contrast = F.relu(logit_neg_g.mean().detach() - logit_fake_for_g.mean() + args.contrast_margin)
                else:
                    contrast = torch.zeros(1, device=device)

                loss_G = (args.lambda_adv * adv + args.lambda_center * center +
                          args.lambda_lm * lm + args.lambda_feat_match * feat_match +
                          args.lambda_pos_dist * psd + args.lambda_contrast * contrast)

                badG = (
                    torch.isnan(loss_G) or torch.isinf(loss_G) or
                    torch.isnan(adv) or torch.isinf(adv) or
                    torch.isnan(lm) or torch.isinf(lm) or
                    torch.isnan(feat_match) or torch.isinf(feat_match) or
                    torch.isnan(psd) or torch.isinf(psd) or
                    torch.isnan(contrast) or torch.isinf(contrast) or
                    torch.isnan(fake_st).any() or torch.isinf(fake_st).any()
                )
                ...
                if args.debug_every > 0 and step % args.debug_every == 0:
                    print(f"[GAN step {step}] loss_D={float(loss_D.item()):.6f} loss_G={float(loss_G.item()):.6f} "
                          f"adv={float(adv.item()):.6f} lm={float(lm.item()):.6f} psd={float(psd.item()):.6f} "
                          f"feat_match={float(feat_match.item()):.6f} contrast={float(contrast.item()):.6f} tau={current_tau:.3f} "
                          f"gnormD={gnormD:.4f} gnormG={gnormG:.4f} bg_miss_total={bg_miss_total}")

                logs.append({"step": step, "epoch": epoch,
                             "loss_D": float(loss_D.item()), "loss_G": float(loss_G.item()),
                             "adv": float(adv.item()), "lm": float(lm.item()), "psd": float(psd.item()),
                             "center": float(center.item()), "feat_match": float(feat_match.item()),
                             "contrast": float(contrast.item()),
                             "tau": float(current_tau), "bg_miss_total": int(bg_miss_total)})
