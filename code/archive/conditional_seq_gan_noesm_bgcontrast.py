...
Goal:
- Generator: takes a bg/cell embedding as input (here bg_vec represents the condition vector) and outputs a 101aa sequence (center position forced to C)
- Discriminator: uses only the token path (Transformer + FiLM(bg)), no longer introducing ESM embeddings
- Data: supports your current parquet format (seq101 + label_bin), and also supports explicit --seq-col / --label-col
"""
...
def load_bg_bank(bg_npy: str, bg_npz: str, protein_table: str, bins: int):
    """
    Returns:
      bg_map: dict[str, np.ndarray] mapping bg_key -> bg_vec
    Supports:
      - packed npz: keys 'samples' (N,) and 'combined_emb' (N,D)  -> bg_map[samples[i]] = combined_emb[i]
      - legacy single-vector: falls back to load_bg_vec(), stored as global__WT1/global__KO1
    """
    if bg_npz and Path(bg_npz).exists():
        z = np.load(bg_npz, allow_pickle=True)
        if set(z.files) >= {"samples", "combined_emb"}:
            ...
            if samples.ndim != 1 or emb.ndim != 2 or emb.shape[0] != samples.shape[0]:
                raise ValueError(f"Invalid bg_npz packed format: samples{samples.shape}, emb{emb.shape}")
            bg_map = {str(samples[i]): emb[i] for i in range(samples.shape[0])}
            return bg_map
        # else: maybe dict-like / single vector; fall through

    # legacy: one global vector
    bg = load_bg_vec(bg_npy, bg_npz, protein_table, bins=bins)
    bg = np.asarray(bg, dtype=np.float32)
    bg = np.nan_to_num(bg, nan=0.0, posinf=0.0, neginf=0.0)
    bg = bg / (np.linalg.norm(bg) + 1e-8)
    bg = np.clip(bg, -5.0, 5.0)
    return {"global__WT1": bg, "global__KO1": bg}


def pick_bg_keys(bg_map: dict, dataset: str, cond: str):
    """
    Pick bg keys matching `${dataset}__${cond}*` with fallback to global.
    cond: WT / KO
    """
    pref = f"{dataset}__{cond}"
    keys = [k for k in bg_map.keys() if str(k).startswith(pref)]
    if not keys:
        pref = f"global__{cond}"
        keys = [k for k in bg_map.keys() if str(k).startswith(pref)]
    if not keys:
        raise ValueError(f"No bg keys for dataset={dataset} cond={cond}. Example keys: {list(bg_map.keys())[:10]}")
    # stable sort by replicate number if present
    def repnum(k):
        s = str(k)
...
class SiteSeqDataset(Dataset):
    def __init__(self, site_table: str, seq_col: str = "seq101", label_col: str = "label_bin", dataset_col: str = "dataset"):
        ...
        # optional dataset column for conditional backgrounds
        if dataset_col in df.columns:
            datasets = df[dataset_col].astype(str).fillna("global").tolist()
        else:
            datasets = ["global"] * len(seqs)
...
def collate(batch: List[dict]):
    ...
    dsets = [b.get("dataset", "global") for b in batch]
    return {"tokens": tokens, "label": labels, "seq101": seqs, "dataset": dsets}
...
def main():
    ...
    ap.add_argument("--lambda-bg", type=float, default=0.2,
                    help="weight for WT-vs-KO background contrast loss on positive sites")
    ap.add_argument("--bg-margin", type=float, default=0.0,
                    help="hinge margin for (logit_WT - logit_KO); 0 means just push WT>KO")
    ap.add_argument("--lambda-bg-neg", type=float, default=0.0,
                    help="optional penalty to keep negatives background-insensitive (|Δlogit|)")
    ...
    # load background bank (WT/KO reps) from bg_npz if available
    bg_map = load_bg_bank(args.bg_npy, args.bg_npz, args.protein_table, bins=args.bins)
    bg_dim = int(next(iter(bg_map.values())).shape[0])

    ds = SiteSeqDataset(args.site_table, seq_col=args.seq_col, label_col=args.label_col, dataset_col="dataset")

    def _collate(b):
        base = collate(b)
        dsets = base.get("dataset", ["global"] * base["tokens"].size(0))
        # pick random WT/KO replicate per sample
        wt_vecs = []
        ko_vecs = []
        used_vecs = []
        used_cond = []
        for ds_name in dsets:
            wt_keys = pick_bg_keys(bg_map, str(ds_name), "WT")
            ko_keys = pick_bg_keys(bg_map, str(ds_name), "KO")
            wt = bg_map[random.choice(wt_keys)]
            ko = bg_map[random.choice(ko_keys)]
            # choose which condition bg to feed the generator/discriminator this step
            ...
            wt_vecs.append(wt)
            ko_vecs.append(ko)
        base["bg_wt"] = torch.tensor(np.stack(wt_vecs, axis=0)).float()
        base["bg_ko"] = torch.tensor(np.stack(ko_vecs, axis=0)).float()
        base["bg"] = torch.tensor(np.stack(used_vecs, axis=0)).float()
        base["bg_cond"] = used_cond
        return base
    ...
    for epoch in range(1, args.epochs + 1):
        for batch in loader:
            ...
            bg_wt = batch["bg_wt"].to(device)
            bg_ko = batch["bg_ko"].to(device)
            # -------------------------
            ...
            # --- Real pos/neg loss computed under BOTH WT and KO backgrounds (forces bg to be used) ---
            loss_D = 0.0
            if mask_pos.any():
                y1 = torch.ones((int(mask_pos.sum()),), device=device)
                loss_D = loss_D + crit(D(real_tok[mask_pos], bg_wt[mask_pos], is_soft=False), y1)
                loss_D = loss_D + crit(D(real_tok[mask_pos], bg_ko[mask_pos], is_soft=False), y1)
            if mask_neg.any():
                y0 = torch.zeros((int(mask_neg.sum()),), device=device)
                loss_D = loss_D + crit(D(real_tok[mask_neg], bg_wt[mask_neg], is_soft=False), y0)
                loss_D = loss_D + crit(D(real_tok[mask_neg], bg_ko[mask_neg], is_soft=False), y0)

            # --- WT vs KO contrast on POS only: encourage logit_WT > logit_KO ---
            bg_contrast = torch.tensor(0.0, device=device)
            if args.lambda_bg > 0 and mask_pos.any():
                logit_wt = D(real_tok[mask_pos], bg_wt[mask_pos], is_soft=False).view(-1)
                logit_ko = D(real_tok[mask_pos], bg_ko[mask_pos], is_soft=False).view(-1)
                diff = logit_wt - logit_ko
                # hinge: max(0, margin - diff)
                bg_contrast = torch.relu(float(args.bg_margin) - diff).mean()
                loss_D = loss_D + float(args.lambda_bg) * bg_contrast

            # optional: keep negatives background-insensitive
            bg_contrast_neg = torch.tensor(0.0, device=device)
            if args.lambda_bg_neg > 0 and mask_neg.any():
                logit_wt_n = D(real_tok[mask_neg], bg_wt[mask_neg], is_soft=False).view(-1)
                logit_ko_n = D(real_tok[mask_neg], bg_ko[mask_neg], is_soft=False).view(-1)
                bg_contrast_neg = (logit_wt_n - logit_ko_n).abs().mean()
                loss_D = loss_D + float(args.lambda_bg_neg) * bg_contrast_neg

            # --- Fake (generated) should be scored as 0 under the SAME bg used to sample ---
            with torch.no_grad():
            ...
            if step % 200 == 0:
                ...
                logs.append({"step": step, "epoch": epoch, "loss_D": float(loss_D.item()), "loss_G": float(loss_G.item()),
                             ...
                             "bg_contrast_pos": float(bg_contrast.item()) if "bg_contrast" in locals() else 0.0,
                             "bg_contrast_neg": float(bg_contrast_neg.item()) if "bg_contrast_neg" in locals() else 0.0})
